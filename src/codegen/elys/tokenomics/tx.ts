...
export interface MsgDeleteGenesisInflation {
  authority: string;
}
export interface MsgDeleteGenesisInflationProtoMsg {
  typeUrl: "/elys.tokenomics.MsgDeleteGenesisInflation";
  value: Uint8Array;
}
export interface MsgDeleteGenesisInflationAmino {
  authority: string;
}
export interface MsgDeleteGenesisInflationAminoMsg {
  type: "/elys.tokenomics.MsgDeleteGenesisInflation";
  value: MsgDeleteGenesisInflationAmino;
}
export interface MsgDeleteGenesisInflationSDKType {
  authority: string;
}
...
function createBaseMsgDeleteGenesisInflation(): MsgDeleteGenesisInflation {
  return {
    authority: ""
  };
}
export const MsgDeleteGenesisInflation = {
  typeUrl: "/elys.tokenomics.MsgDeleteGenesisInflation",
  encode(message: MsgDeleteGenesisInflation, writer: BinaryWriter = BinaryWriter.create()): BinaryWriter {
    if (message.authority !== "") {
      writer.uint32(10).string(message.authority);
    }
    return writer;
  },
  decode(input: BinaryReader | Uint8Array, length?: number): MsgDeleteGenesisInflation {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseMsgDeleteGenesisInflation();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromPartial(object: Partial<MsgDeleteGenesisInflation>): MsgDeleteGenesisInflation {
    const message = createBaseMsgDeleteGenesisInflation();
    message.authority = object.authority ?? "";
    return message;
  },
  fromAmino(object: MsgDeleteGenesisInflationAmino): MsgDeleteGenesisInflation {
    return {
      authority: object.authority
    };
  },
  toAmino(message: MsgDeleteGenesisInflation): MsgDeleteGenesisInflationAmino {
    const obj: any = {};
    obj.authority = message.authority;
    return obj;
  },
  fromAminoMsg(object: MsgDeleteGenesisInflationAminoMsg): MsgDeleteGenesisInflation {
    return MsgDeleteGenesisInflation.fromAmino(object.value);
  },
  fromProtoMsg(message: MsgDeleteGenesisInflationProtoMsg): MsgDeleteGenesisInflation {
    return MsgDeleteGenesisInflation.decode(message.value);
  },
  toProto(message: MsgDeleteGenesisInflation): Uint8Array {
    return MsgDeleteGenesisInflation.encode(message).finish();
  },
  toProtoMsg(message: MsgDeleteGenesisInflation): MsgDeleteGenesisInflationProtoMsg {
    return {
      typeUrl: "/elys.tokenomics.MsgDeleteGenesisInflation",
      value: MsgDeleteGenesisInflation.encode(message).finish()
    };
  }
};
